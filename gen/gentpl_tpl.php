<?php
include("{cfdir_inc}gentpl_includes.php");
$tt=array(
	...
	'ttarrayname'=>'{ttarrayname}',
	);
include_once ("{ttfilename}") ;
/* debut code spe inc */{tablename_inc}/* fin code spe */
...
/* debut code spe head */{tablename_head}/* fin code spe */
...
if (($action=="edit") or ($action=="mod") or ($action=="add"))  //---------------------------------EDIT----------------------------------------
{
	...
	/* debut code spe headf */{tablename_headf}/* fin code spe */
	...
	if($tt["form"])
	  foreach($tt["form"] as $ttfield=>$ttv){
		...
		/* debut code spe loopf */{tablename_loopf}/* fin code spe */
		}
	/* debut code spe endf */{tablename_endf}/* fin code spe */
}
...
/* debut code spe search */{tablename_search}/* fin code spe */
...
$sqlorderby=" order by ".
		(($_REQUEST["orderby"])?$_REQUEST["orderby"]:
		($tb[$tbid]["orderby"])?$tb[$tbid]["orderby"]:$tbid).
		(($_REQUEST["orderby_desc"])?" desc ":" ");
...
/* debut code spe headp */{tablename_headp}/* fin code spe */
...
for ($i=0;$i<count($ttlist["list"]);$i++)
{
	...
	$ttlist["list"][$i]=$ttv;
	/* debut code spe loop */{tablename_loop}/* fin code spe */
}
$tt["divlist"]=$ttlist;
$tt["multimod"]=$ttlist;
foreach(array_keys($tb) as $tbk)
...
/* debut code spe endp */{tablename_endp}/* fin code spe */
/* debut code spe end */{tablename_end}/* fin code spe */

print gentpl_template($tt);
